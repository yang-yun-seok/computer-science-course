(()=>{
 ...
 Object.entries(CS.lessons).forEach(([lessonId,lesson],lessonIndex)=>{
  ...
  authored.forEach((item,index)=>{
   ...
   questions.push({id,revision:2,lessonId,type:'choice',conceptTags:[item.tag||'application'],prompt:item.prompt,options:choices.map((text,optionIndex)=>({id:`${id}-o${optionIndex+1}`,text})),correctOptionId:`${id}-o${correctIndex+1}`,feedback:choices.map((_,optionIndex)=>optionIndex===correctIndex?item.why:`이 선택은 상황의 핵심 조건과 맞지 않아요. ${item.why}`),reviewTargets:[{lessonId,sectionId:lesson.sections[index]?.id||'summary'}]});
  });
 });
