...
const scripts=['src/core.js',...fs.readdirSync(path.join(root,'src/labs')).filter(x=>x.endsWith('.js')).sort().map(x=>'src/labs/'+x),...fs.readdirSync(path.join(root,'src/assessment')).filter(x=>x.endsWith('.js')).sort().map(x=>'src/assessment/'+x),...fs.readdirSync(path.join(root,'content')).filter(x=>x.endsWith('.js')).sort().map(x=>'content/'+x),'src/app.js'];
