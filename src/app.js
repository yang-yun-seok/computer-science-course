...
function renderGlossary(){
 ...
 main.innerHTML=`<article class="glossary"><header class="glossary-hero"><div class="section-kicker">부록 다음에 다시 찾아보는</div><h1 tabindex="-1">용어 정리</h1><p>낯선 단어를 검색하고, 어느 과목의 어떤 주제인지 확인한 뒤 관련 교안으로 돌아가세요. 용어는 외울 목록이 아니라 개념 사이를 잇는 표지판이에요.</p><dl>${glossary.subjects.map(subject=>`<div><dt>${e(subject.label)}</dt><dd>${glossary.terms.filter(term=>term.subject===subject.id).length.toLocaleString()}</dd></div>`).join('')}<div><dt>전체</dt><dd>${glossary.terms.length.toLocaleString()}</dd></div></dl></header><section class="glossary-guide" aria-labelledby="glossary-guide-title"><div><span>읽는 방법</span><h2 id="glossary-guide-title">이름 → 쓰임 → 교안 순서로 확인해요</h2></div><ol><li><b>1</b>한글·영문 용어를 함께 읽어요.</li><li><b>2</b>과목과 세부 주제로 쓰임을 좁혀요.</li><li><b>3</b>연결된 교안에서 원리와 실습을 확인해요.</li></ol></section><section class="glossary-browser" aria-labelledby="glossary-index-title"><div class="glossary-tools"><div><div class="section-kicker">키워드 찾아보기</div><h2 id="glossary-index-title">${glossary.terms.length.toLocaleString()}개 용어</h2></div><label class="glossary-search"><span class="sr-only">용어 검색</span><input type="search" placeholder="예: 가상 메모리, TCP, index" autocomplete="off" spellcheck="false"><kbd>/</kbd></label><div class="glossary-filters" role="group" aria-label="과목 필터"><button type="button" class="active" data-subject="all">전체</button>${glossary.subjects.map(subject=>`<button type="button" data-subject="${subject.id}">${e(subject.label)}</button>`).join('')}</div></div><p class="glossary-result" role="status"></p><div class="glossary-list"></div><button type="button" class="glossary-more">용어 더 보기</button></section><aside class="glossary-source"><strong>참고 범위</strong><p>${e(glossary.source.note)} 현재 교안에서는 검색·분류·관련 단원 연결 방식으로 다시 구성했습니다.</p><p><a href="${e(glossary.source.site)}" target="_blank" rel="noopener noreferrer">CSnote 웹사이트</a><span aria-hidden="true"> · </span><a href="${e(glossary.source.repository)}" target="_blank" rel="noopener noreferrer">공개 저장소</a></p></aside></article>`;
 ...
 function paint(){
  const query=normalize(search.value),filtered=glossary.terms.filter(term=>(active==='all'||term.subject===active)&&(!query||normalize(`${term.term} ${term.english} ${term.topic} ${term.detail}`).includes(query)));
