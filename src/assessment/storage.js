(()=>{
 ...
 function addReview(question,source,selected){const items=reviews(),existing=items.find(item=>item.question.id===question.id);if(existing){existing.attempts=(existing.attempts||0)+1;existing.lastSelected=selected??null;existing.updatedAt=Date.now();existing.status='open';existing.source=source;}else items.unshift({id:`review-${question.id}`,question:assessment.clone(question),source,attempts:1,lastSelected:selected??null,createdAt:Date.now(),updatedAt:Date.now(),completedAt:null,status:'open'});return saveReviews(items);}
