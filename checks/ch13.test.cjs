...
test('query plan presets cover scan, index, key, expression and join',async()=>{const l=labs['query-plan'];for(const id of ['scan','index','primary','expression','join']){const s=await l.action(l.initial(),'preset-'+id);assert.equal(s.preset,id);assert.match(s.sql,/EXPLAIN QUERY PLAN/);}const scan=await l.action(l.initial(),'preset-scan'),index=await l.action(l.initial(),'preset-index');assert.match(index.sql,/CREATE INDEX/);assert.equal((scan.sql.match(/SELECT \*/g)||[]).length,2);assert.equal((index.sql.match(/SELECT \*/g)||[]).length,2);});
